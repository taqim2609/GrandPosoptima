...
DIR="$(pwd)"
mkdir -p backups
chmod +x backup-pi.sh backup-to-cloud.sh backup-to-vibecoder.sh 2>/dev/null || true
LINE="0 23 * * * cd $DIR && ./backup-to-cloud.sh >> $DIR/backups/autobackup.log 2>&1"
( crontab -l 2>/dev/null | grep -v 'backup-pi.sh' | grep -v 'backup-to-vibecoder.sh' ; echo "$LINE" ) | crontab -
